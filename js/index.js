...
let displayData = (displayRecivedData) => {
  displayRecivedData.forEach((singleNewsData) => {
    ...
    modalBody.innerHTML = ` 
     ...
     <p class="modal-text"> ${singleNewsData.details.slice(0, 650)} </p>
   `;
    // modal end
...
let showNewsInHml = (getNewsCategorized) => {
  ...
  for (let singleNewsCategory of getNewsCategorized) {
    ...
    modalBody.innerHTML = ` 
      ...
      <p class="modal-text"> ${singleNewsCategory.details.slice(0, 650)} </p>
    `;
    // modal end
...
let spinnerBodoy = document.getElementById("spinner-body");
function spinner(isLoading) {
  if (isLoading === true) {
    spinnerBodoy.classList.remove("d-none");
  } else {
    spinnerBodoy.classList.add("d-none");
  }
}
